...
class PropertyRemoteDataSource {
  ...
  // Helper conversion for collection references
  CollectionReference<Map<String, dynamic>> get _propertiesCollection =>
      _firestore.collection(FirestoreKeys.listingsCollection);
  ...
  Future<List<PropertyModel>> getFeaturedProperties() async {
    // Assumes you have an 'isFeatured' boolean or status tracking on Firestore documents
  ...
  Future<List<PropertyModel>> searchProperties(String query) async {
    ...
    // Note: Firestore does not support case-insensitive contains natively.
    // For a simple mock search matching small datasets, fetch properties and filter locally.
    // For large production datasets, consider using Algolia, ElasticSearch, or a firestore array-keywords field.
    final snapshot = await _propertiesCollection.get();
